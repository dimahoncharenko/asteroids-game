...
window.addEventListener("DOMContentLoaded", () => {
    ...
    if (canvas) {
        ...
        function drawShip(x, y, dir, color = "white") {
        ...
        function newAsteroid(x, y, r = Math.ceil(ASTEROID_SIZE / 2)) {
            let asteroid = {
                x,
                y,
                ...
                xv: Math.random() *
                    ASTEROID_SPD *
                    level *
                    Math.cos(Math.random() * (Math.PI * 2) + (Math.random() > 0.5 ? 1 : -1)),
                yv: Math.random() *
                    ASTEROID_SPD *
                    level *
                    Math.sin(Math.random() * (Math.PI * 2) + (Math.random() > 0.5 ? 1 : -1)),
        ...
        function drawAsteroids() {
            for (let i = 0; i < asteroids.length; i++) {
                ...
                ctx.lineWidth = SHIP_SIZE / 20;
                ctx.beginPath();
                ctx.moveTo(asteroids[i].x +
                    asteroids[i].offsets[0] *
                        asteroids[i].r *
                        Math.cos(asteroids[i].dir), asteroids[i].y +
                    asteroids[i].offsets[0] *
                        asteroids[i].r *
                        Math.sin(asteroids[i].dir));
                for (let j = 1; j < asteroids[i].vert; j++) {
                    ctx.lineTo(asteroids[i].x +
                        asteroids[i].offsets[j] *
                            asteroids[i].r *
                            Math.cos(asteroids[i].dir + (j * Math.PI * 2) / asteroids[i].vert), asteroids[i].y +
                        asteroids[i].offsets[j] *
                            asteroids[i].r *
                            Math.sin(asteroids[i].dir + (j * Math.PI * 2) / asteroids[i].vert));
                }
                ctx.closePath();
                ctx.stroke();
                ...
                if (SHOW_COLLISION_MASK) {
                    ...
                    ctx.lineWidth = SHIP_SIZE / 20;
                    ctx.beginPath();
                    ctx.arc(asteroids[i].x, asteroids[i].y, asteroids[i].r, Math.PI * 2, 0);
                    ctx.stroke();
                }
            }
        }
        ...
        function shootLaser() {
            if (!ship.canShoot || ship.lasers.length >= LASER_NUM)
                return;
            fxLaser.play();
            ship.lasers.push({
        ...
        function draw() {
            ...
            if (ship.isThrusting && !ship.isDead) {
                if (!isExploded) {
                    ...
                    if (isShowed) {
                        ...
                        ctx.beginPath();
                        ctx.moveTo(ship.x - ship.r * (Math.cos(ship.dir) + 0.5 * Math.sin(ship.dir)), ship.y + ship.r * (Math.sin(ship.dir) - 0.5 * Math.cos(ship.dir)));
                        ctx.lineTo(ship.x - ship.r * ((6 / 3) * Math.cos(ship.dir)), ship.y + ship.r * ((6 / 3) * Math.sin(ship.dir)));
                        ctx.lineTo(ship.x - ship.r * (Math.cos(ship.dir) - 0.5 * Math.sin(ship.dir)), ship.y + ship.r * (Math.sin(ship.dir) + 0.5 * Math.cos(ship.dir)));
                        ctx.closePath();
                        ctx.stroke();
                        ctx.fill();
                    }
            ...
            if (!isExploded) {
                ...
                if (ship.blinkNum === 0 && !ship.isDead) {
                    ...
                    for (let i = 0; i < asteroids.length; i++) {
                        if (distanceBetweenPoints(ship.x, ship.y, asteroids[i].x, asteroids[i].y) <
            ...
            for (let i = ship.lasers.length - 1; i >= 0; i--) {
                ...
                ctx.beginPath();
                ctx.arc(ship.lasers[i].x, ship.lasers[i].y, SHIP_SIZE / 20, Math.PI * 2, 0);
                ctx.fill();
            ...
            for (let i = ship.lasers.length - 1; i >= 0; i--) {
                ...
                for (let j = 0; j < asteroids.length; j++) {
                    ...
                    if (distanceBetweenPoints(laser.x, laser.y, asteroid.x, asteroid.y) <
                        asteroid.r) {
                        ship.lasers.splice(i, 1);
                        destroyAsteroid(j);
                        break;
